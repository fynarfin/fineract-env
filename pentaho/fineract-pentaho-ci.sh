...
CodeDir="${HOME}/Documents/Fynarfin/Code/test/"
...
RUN ./gradlew --no-daemon -q -x rat -x compileTestJava -x test -x spotlessJavaCheck -x spotlessJava bootJar --stacktrace
...
# Build image
docker build -t fineract:pentaho-enabled-fineract .
